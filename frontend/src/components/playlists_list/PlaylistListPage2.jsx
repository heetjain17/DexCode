...
const companies = [
  {
    name: 'Google',
    playlists: ['Dynamic Programming', 'Recursion & Backtracking', 'Core DSA'],
    accent: 'text-blue-400',
    gradient: 'bg-gradient-to-tr from-blue-900/40 to-neutral-950',
    logo: GoogleLogo
  },
  {
    name: 'Amazon',
    playlists: ['Array & Strings', 'Trees & Graphs', 'System Design'],
    accent: 'text-orange-400',
    gradient: 'bg-gradient-to-tr from-orange-900/40 to-neutral-950',
    logo: AmazonLogo
  },
  {
    name: 'Microsoft',
    playlists: ['Linked Lists', 'Heaps & Queues'],
    accent: 'text-sky-400',
    gradient: 'bg-gradient-to-tr from-sky-900/40 to-neutral-950',
    logo: MicrosoftLogo
  }
];
const PlaylistListPage2 = () => {
  return (
    <div className="bg-neutral-950 text-white min-h-screen px-4 py-10 sm:px-8">
      {/* Header */}
      <header className="max-w-4xl mx-auto text-center mb-20">
        <h1 className="text-5xl sm:text-6xl font-bold text-transparent bg-clip-text bg-gradient-to-b from-white to-neutral-500 pb-1">
          Explore Company Playlists
        </h1>
        <p className="mt-4 text-neutral-400 text-lg">
          Dive into curated problem sets from top tech companies.
        </p>
      </header>
      ...
      <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 max-w-7xl mx-auto">
        {companies.map(company => (
          <GlareCard
            key={company.name}
            className={`p-6 rounded-2xl h-full w-full ${company.accent} ${company.gradient}`}
          >
            <div className="flex flex-col h-full justify-between">
              <div className="flex items-center justify-between">
                <h2 className="text-xl font-bold text-white">{company.name}</h2>
                <span className={`text-sm font-medium ${company.accent}`}>
                  +{company.playlists.length} Sets
                </span>
              </div>

              <ul className="space-y-2 mt-4 text-sm text-neutral-300">
                {company.playlists.slice(0, 3).map((title, index) => (
                  <li
                    key={index}
              ...
              <a
                href="#"
                className={`inline-flex items-center text-sm font-semibold group mt-4 ${company.accent}`}
              >
...
export default PlaylistListPage2;
