...
const AmazonLogo = () => (
  ...
  >
    ...
    <g id="SVGRepo_iconCarrier">
      {' '}
      <title>Amazon-color</title> <desc>Created with Sketch.</desc> <defs> </defs>{' '}
...
// --- Mock Data with Theme Information ---
const companyPlaylists = [
  {
    companyName: 'Google',
    LogoComponent: GoogleLogo,
    theme: {
      gradient: 'from-blue-900/40 to-neutral-950',
      accentColor: 'text-blue-400'
    },
    playlists: [
      { id: 'g-01', title: 'Dynamic Programming Essentials', problemCount: 18 },
      { id: 'g-02', title: 'Advanced Recursion & Backtracking', problemCount: 12 },
      { id: 'g-03', title: 'Core Data Structures', problemCount: 25 }
    ]
  },
  {
    companyName: 'Amazon',
    ...
    theme: {
      gradient: 'from-orange-900/40 to-neutral-950',
      accentColor: 'text-orange-400'
    },
    playlists: [
      { id: 'a-01', title: 'Array & String Fundamentals', problemCount: 22 },
      { id: 'a-02', title: 'Trees & Graph Traversal', problemCount: 15 },
      { id: 'a-03', title: 'System Design Primitives', problemCount: 8 }
    ]
  },
  {
    ...
    theme: {
      gradient: 'from-sky-900/40 to-neutral-950',
      accentColor: 'text-sky-400'
    },
    playlists: [
      { id: 'm-01', title: 'Linked List Mastery', problemCount: 17 },
      { id: 'm-02', title: 'Heaps & Priority Queues', problemCount: 9 }
    ]
  }
];
...
const PlaylistListPage = () => {
  return (
    <div className="bg-neutral-950 text-white min-h-screen p-4 sm:p-8">
      <div className="max-w-7xl mx-auto">
        <header className="text-center mb-16">
          <h1 className="text-4xl sm:text-5xl font-bold tracking-tight bg-clip-text text-transparent bg-gradient-to-b from-neutral-50 to-neutral-400">
            Company Playlists
          </h1>
          <p className="mt-4 text-lg text-neutral-400 max-w-2xl mx-auto">
            Curated problem sets based on frequently asked topics at top tech companies.
          </p>
        </header>

        <div className="space-y-20">
          {companyPlaylists.map(company => (
            <section key={company.companyName}>
              <h2 className="text-3xl font-bold mb-8 flex items-center gap-4 text-neutral-200">
                {company.companyName}
              </h2>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {company.playlists.map(playlist => (
                  <GlareCard key={playlist.id} className="p-0">
                    {/* The content below is passed as `children` to GlareCard */}
                    {/* This inner div receives the custom gradient and contains all content */}
                    <div
                      className={`relative w-full h-full bg-gradient-to-br ${company.theme.gradient} p-6 flex flex-col`}
                    >
                      {/* Logo as a watermark */}
                      <div
                        className={`rotate-12 absolute top-8 right-8 opacity-10 mix-blend-luminosity ${company.theme.accentColor}`}
                      >
                        <company.LogoComponent />
                      </div>

                      <div className="flex-grow z-10">
                        <div className="p-2 bg-red-500/10 rounded-full w-10 h-10 flex items-center justify-center mb-4 border border-red-500/20">
                      ...
                      <a
                        ...
                        className={`mt-8 flex items-center justify-between ${company.theme.accentColor} font-semibold group z-10`}
                      >
